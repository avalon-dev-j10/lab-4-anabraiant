...
public class SortClass implements Sort{
    ...
    @Override
    public void sort(Object[] array, Comparator comparator) { //реализация компоратора такая же как Comparable
        for (int i =0; i < array.length; i++){
            for (int j = 0; j < array.length; j++){
                if( comparator.compare(array[j], array[j+1]) == -1 ){ ////проверка объекта, если он меньше, чем следущий, то меняем местами
